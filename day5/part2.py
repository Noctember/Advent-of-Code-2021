class Segment:
    ...
    def points(self):
        if self.end[0] - self.start[0] != 0 and self.end[1] - self.start[1] != 0:
            s1 = int(self.end[0] > self.start[0]) or -1
            s2 = int(self.end[1] > self.start[1]) or -1
            points = []
            for i in range(abs(self.end[0] - self.start[0]) + 1):
                x = s1 * i + self.start[0]
                y = s2 * i + self.start[1]
                points.append((x, y))
            return points
            ...
            return [(self.start[0], y) for y in
                    range(min(self.start[1], self.end[1]), max(self.start[1], self.end[1]) + 1)]
        else:
            return [(x, self.start[1]) for x in
                    range(min(self.start[0], self.end[0]), max(self.start[0], self.end[0]) + 1)]
